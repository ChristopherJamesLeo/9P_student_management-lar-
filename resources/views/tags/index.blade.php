        ...
        <div>
            <form action="{{route('tags.store')}}" method="POST">
                ...
                <div class="mb-2 form-group">
                    <input type="text" name="name" id="name" class="form-control rounded-0 border outline-none shadow-none" placeholder="Tag" value="{{old('name')}}">
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-checkbox form-switch">
                                                <input type="checkbox" name="" id="" class="form-check-input shadow-none outline-none change-btn" {{$tag->status_id == 3 ? "checked" : " "}} data-id = {{$tag->id}}>
                                            </div>
                                        </td>
                                        <td>{{$tag->user["name"]}}</td>
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                ...
                                                data-id = {{$tag->id}}
                                                data-name = {{$tag->name}}
                                                data-status-id = {{$tag->status_id}}
                                                data-bs-toggle = "modal"
                                                data-bs-target = "#editmodal"
                                                 class="btn btn-outline-primary btn-sm edit_form_btn"><i class="fas fa-edit"></i></a>
                                                <a href="javascript:void(0)" class="btn btn-danger btn-sm delete_btn" data-id={{$tag->id}}><i class="fas fa-trash"></i></a>
                                            </div>
                                        </td>
...
<script>
    $(document).ready(function(){
        ...
        $(".edit_form_btn").click(function(){
            ...
            $("#editform").attr("action",`/tags/${getId}`)
